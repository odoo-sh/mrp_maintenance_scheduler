...
class MaintenanceEquipmentStep(models.Model):
    ...
    def _compute_next_maintenance(self):
        ...
        equipments = self.filtered(lambda x: x.maintenance_operation_id.period > 0)
        for equipment in equipments:
            ...
            if next_maintenance_todo and last_maintenance_done:
                ...
                if date_gap > timedelta(0) and date_gap > timedelta(days=equipment.maintenance_operation_id.period) * 2 and next_maintenance_todo.request_date > date_now:
                    # If the new date still in the past, we set it for today
                    if last_maintenance_done.close_date + timedelta(days=equipment.maintenance_operation_id.period) < date_now:
                        next_date = date_now
                    else:
                        next_date = last_maintenance_done.close_date + timedelta(days=equipment.maintenance_operation_id.period)
                ...
                # We use 2 times the period to avoid creation too closed request from a manually one created
                if date_gap > timedelta(0) and date_gap > timedelta(days=equipment.maintenance_operation_id.period) * 2:
                    next_date = date_now + timedelta(days=equipment.maintenance_operation_id.period)
                ...
                next_date = equipment.maintenance_equipment_id.effective_date + timedelta(days=equipment.maintenance_operation_id.period)
